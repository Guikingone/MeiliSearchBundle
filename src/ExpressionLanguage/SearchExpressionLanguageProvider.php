...
final class SearchExpressionLanguageProvider implements ExpressionFunctionProviderInterface
{
    ...
    {
        return [
            new ExpressionFunction('search', function (string $index, string $query, ?string $filters = 'empty', ?int $max = null): string {
                return sprintf('IN %s ON %s WHERE %s MAX %d', $index, $query, $filters, $max);
            }, function (array $arguments, string $index, string $query, ?string $filters = null, ?int $max = null): Search {
